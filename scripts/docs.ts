...
function checkDocs(): void {
  const allFiles = listMarkdownFiles(docsRoot).filter(
    (file) => !file.includes(`${path.sep}_templates${path.sep}`)
  )
  const files = allFiles.filter((file) => !isLegacyRecordDraft(file))

  // ponytail: legacy drafts predate doc governance; remove this exemption when those directories are migrated or deleted.
  const skipped = allFiles.length - files.length
  if (skipped > 0) {
    warn(`Skipped ${skipped} legacy record drafts without frontmatter`)
  }
...
function generateIndex(): void {
  ...
  const files = listMarkdownFiles(docsRoot)
    ...
    .filter((file) => !isLegacyRecordDraft(file))
    .sort()
...
function isLegacyRecordDraft(file: string): boolean {
  const rel = relative(file)
  return (
    (rel.startsWith('docs/record/formily/') ||
      rel.startsWith('docs/record/phase/')) &&
    parseFrontmatter(fs.readFileSync(file, 'utf8')) === null
  )
}
