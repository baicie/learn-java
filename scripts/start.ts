...
function printStatus(): void {
  ...
  console.log(check('Java 21',          'java -version 2>&1 | findstr "21"',   'java -version 2>&1 | grep 21'))
  console.log(check('Maven',            'mvn -version 2>&1 | findstr Maven',  'mvn -version 2>&1 | grep Maven'))
  console.log(check('Node.js',           'node --version',                       'node --version'))
  console.log(check('Docker',            'docker --version 2>nul',                'docker --version 2>/dev/null'))
  console.log(check('Docker Compose',    'docker compose version 2>nul || docker-compose --version 2>nul', 'docker compose version 2>/dev/null || docker-compose --version 2>/dev/null'))

  printHeader('Docker Containers')
  for (const svc of ['postgres', 'redis', 'clickhouse', 'victoriametrics', 'minio']) {
    ...
    console.log(running ? `  \u2713 ${svc} (running)` : `  \u2717 ${svc} (stopped)`)
  }
...
async function buildBackend(): Promise<void> {
  ...
  const mvnCmd = isWin ? 'mvn.cmd clean install -DskipTests' : 'mvn clean install -DskipTests'
  await run(mvnCmd)
...
async function startApp(key: keyof typeof APPS): Promise<void> {
  ...
  const jar = join(javaRoot, app.dir, 'target', `${app.name}-0.1.0-SNAPSHOT.jar`)
  if (!existsSync(jar)) {
  ...
  console.log(`  \u25b6 ${app.name} started (PID: ${pid.pid}) -> http://localhost:${app.port}`)
}
...
async function stopApp(key: keyof typeof APPS): Promise<void> {
  const { existsSync: exists, readFileSync: read, unlinkSync: rm } = await import('node:fs')
  const pidFile = join(root, `.pid-${key}`)
...
async function stopAll(): Promise<void> {
  ...
  console.log('  (Docker containers still running — use "tsx scripts/start.ts infra-stop" to stop infra)')
}
...
Commands:
  ...
  backend    Build Maven project and start all three backend apps
  frontend   Start frontend dev server
  all        Start infra + backend (default)
  ...
  switch (command) {
    ...
    case 'logs': {
      ...
      if (!APPS[target as keyof typeof APPS]) {
        console.error(`Unknown app: ${app}. Use: ${Object.keys(APPS).join(' | ')}`)
        return
      }
