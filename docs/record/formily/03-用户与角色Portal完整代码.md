...
export async function replacePlatformUserRoles(
  ...
  const response = await apiClient.put(
    `/api/platform/users/${userId}/roles`,
    input
  )
  return platformUserSchema.parse(unwrapApiResponse(response.data))
}
...
export const platformUserKeys = {
  ...
  list: (query: PlatformUserQuery) =>
    [...platformUserKeys.lists(), query] as const,
  detail: (id: string) =>
    [...platformUserKeys.all, 'detail', id] as const,
}
```
...
export const platformRoleSchema = z.object({
  ...
  dataScopes: z.record(z.string(), z.object({
    ...
    <>
      ...
      <Main className='min-h-0 flex-1 overflow-hidden p-0'>
        <div className='grid h-full min-h-0 grid-cols-[280px_minmax(0,1fr)]'>
          ...
          <RoleEditor
            editor={editor}
            permissionTree={permissions.data ?? []}
          />
        </div>
      </Main>
    </>
      ...
      <div className='flex items-center gap-3 border-b p-3'>
        <Checkbox
          ...
          onCheckedChange={(checked) =>
            onChange(childCodes, checked === true)
          }
        />
        <div>
