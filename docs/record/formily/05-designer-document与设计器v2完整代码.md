---
title: DesignerDocument、编译器与设计器 V2 完整代码
type: design
status: draft
phase: work-record
owner: ai
...
export type DesignerNode =
  DesignerFieldNode | DesignerSectionNode | DesignerGridNode | DesignerDividerNode
...
export const designerFieldNodeSchema = baseNodeSchema.extend({
  ...
  fieldCode: z.string().regex(/^[a-zA-Z][a-zA-Z0-9_]{0,63}$/),
...
export function compileDesignerDocument(document: DesignerDocument): ISchema {
...
function compileField(node: DesignerFieldNode, index: number): ISchema {
  return {
    ...
    'x-work-record': {
      ...
      optionSource: node.dataSource.type === 'dict' ? 'dict' : 'static',
      dictCode: node.dataSource.type === 'dict' ? node.dataSource.dictCode : undefined,
      ...node.capability,
    },
  }
}
  ...
  walkNodes(document.nodes, (node) => {
    ...
    if (node.kind === 'field') {
      if (!fieldCodes.add(node.fieldCode)) {
        throw new DesignerValidationError(`重复字段编码：${node.fieldCode}`)
      }
...
export const useDesignerStore = create<DesignerStore>((set, get) => ({
  ...
  addNode: (parentId, index, node) => {
    commit(set, get, (document) => insertNode(document, parentId, index, node))
  ...
  moveNode: (id, parentId, index) => {
    ...
    commit(set, get, (current) => moveDesignerNode(current, id, parentId, index))
  },

  updateNode: (id, patch) => {
    commit(set, get, (document) => updateDesignerNode(document, id, patch))
  },

  duplicateNode: (id) => {
    const copy = cloneDesignerNode(requireNode(get().document, id))
    commit(set, get, (document) => insertAfterNode(document, id, copy))
...
type DesignerDragData =
  { type: 'library'; fieldType: WorkRecordFieldType } | { type: 'canvas-node'; nodeId: string }
...
export function WorkRecordDesignerV2Page() {
  ...
  const compiledSchema = useMemo(() => compileDesignerDocument(document), [document])
...
export function DesignerWorkspace({ children }: PropsWithChildren) {
  return (
    <ScrollArea className="min-w-0 bg-muted/40">
      <div className="mx-auto min-h-full p-8 xl:p-12">{children}</div>
    </ScrollArea>
  )
}
...
```tsx
export function DesignerCanvasNode({ node, children }: PropsWithChildren<{ node: DesignerNode }>) {
  ...
  return (
    ...
    >
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">支持字段、分组、两列布局和分割线</p>
      </div>
    </div>
  )
}
...
describe('designer store', () => {
  ...
  it('rejects moving layout into descendant', () => {
    ...
    expect(() =>
      useDesignerStore.getState().moveNode(document.nodes[0].id, nestedChildId(document), 0)
    ).toThrow(/descendant/)
  })
})
```
