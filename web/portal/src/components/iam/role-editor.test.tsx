...
import type { PlatformRole } from '@/lib/iam/platform-role'
...
function Harness({ initialCode }: HarnessProps) {
  ...
  return (
    <div>
      ...
      <button type='button' data-testid='reset' onClick={() => editor.reset()}>
        reset
      </button>
    </div>
  )
...
describe('useRoleEditor', () => {
  ...
  it('marks dirty when permissions change', async () => {
    ...
    await act(async () => {
      ...
      ;(
        screen.getByTestId('change-write').element() as HTMLButtonElement
      ).click()
    })
    await expect.element(screen.getByTestId('dirty')).toHaveTextContent('true')
    await expect
      .element(screen.getByTestId('has-write'))
  ...
  it('flags dangerous added codes', async () => {
    ...
    await act(async () => {
      ;(screen.getByTestId('apply').element() as HTMLButtonElement).click()
      ;(
        screen.getByTestId('change-dangerous').element() as HTMLButtonElement
      ).click()
    })
    await expect
      .element(screen.getByTestId('dangerous'))
      .toHaveTextContent('platform:user:status')
  })
