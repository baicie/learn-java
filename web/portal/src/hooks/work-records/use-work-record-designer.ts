...
import { listDictTypes } from '@/api/dictionaries'
import {
...
export function useWorkRecordDesigner(templateId: string) {
  ...
  const selectedTemplate = useMemo(() => {
    ...
    return list.find((item) => item.id === selectedTemplateId)
  ...
  useEffect(() => {
    ...
    const merged = mergePublishedLocks(
      ...
      publishValidation?.referencedRecordCount ?? 0,
      parsed.length === 0 &&
        selectedTemplate.draftSchemaJson.trim() === '{}' &&
        selectedTemplate.draftDesignerJson.trim() === '{}'
    )
