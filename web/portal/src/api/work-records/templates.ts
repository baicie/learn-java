...
} from '@/components/work-records/designer/types'
...
export async function createTemplate(input: {
  ...
  const { data } = await apiClient.post('/api/work-record/templates', {
    ...input,
...
export async function updateTemplate(
  ...
  const { data } = await apiClient.put(
    `/api/work-record/templates/${templateId}`,
    input
  )
  return apiResponseSchema(templateSchema).parse(data).data
}
...
export async function copyTemplate(
  templateId: string,
  input: { targetCode: string; targetName: string; description?: string }
): Promise<WorkRecordTemplate> {
  const { data } = await apiClient.post(
    `/api/work-record/templates/${templateId}/copy`,
    input
  )
  return apiResponseSchema(templateSchema).parse(data).data
}

export const enableTemplate = (templateId: string) =>
  changeTemplateStatus(templateId, 'enable')
