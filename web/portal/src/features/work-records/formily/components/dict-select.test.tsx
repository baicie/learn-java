...
const wrap = (ui: React.ReactNode, options?: { mode?: 'create' | 'edit' | 'readonly'; dictOptions?: Record<string, { value: string; label: string; enabled: boolean }[]>; initialValues?: Record<string, unknown> }) => (
  ...
  >
    <FormProvider form={createWorkRecordForm({ mode: options?.mode ?? 'create', initialValues: options?.initialValues })}>
      {ui}
    </FormProvider>
  </WorkRecordFormRuntimeProvider>
)
