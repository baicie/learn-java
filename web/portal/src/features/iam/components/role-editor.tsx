...
import { useReplaceRolePermissions } from '../hooks/use-platform-roles'
import type {
...
export type RoleEditorSnapshot = {
  role: PlatformRole
...
// to the role-fetch query.
  ...
  const isDirty = useMemo(() => {
    ...
    if (snapshot.selectedPermissions.size !== snapshot.initialPermissions.size) {
      return true
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    if (dangerous) {
      const accepted = await confirm({
        title: '危险权限确认',
        description:
          '本次修改包含 critical 权限，必须明确确认后才可保存。',
        confirmText: '确认保存',
        cancelText: '取消',
        variant: 'destructive',
      })
