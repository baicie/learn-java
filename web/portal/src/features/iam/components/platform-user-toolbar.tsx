  ...
  return (
    <div className='flex flex-wrap items-center gap-2 rounded-md border bg-card/40 p-3'>
      <Input
        ...
        onBlur={(event) => onChange({ keyword: event.target.value || undefined })}
        onKeyDown={(event) => {
          if (event.key === 'Enter') {
            onChange({ keyword: (event.target as HTMLInputElement).value || undefined })
          }
        }}
      ...
      <Select
        ...
        onValueChange={(next) =>
          onChange({
            status:
              next === 'all' ? undefined : (next as PlatformUserQuery['status']),
          })
        }
      >
