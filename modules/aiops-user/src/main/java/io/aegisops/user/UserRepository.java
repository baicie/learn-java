...
public class UserRepository {
    ...
    public UserAccount create(String tenantId, String username, String displayName, String email, String passwordHash) {
        ...
        jdbc.update("""
                ...
                values (?, ?, ?, ?, ?, ?, 'active')
                """, id, tenantId, username, displayName, email, passwordHash);
    ...
    public void attachRole(String userId, String roleCode) {
        ...
        Integer count = jdbc.queryForObject("select count(*) from sys_user_role where user_id = ? and role_id = ?", Integer.class, userId, roleId);
        if (count == null || count == 0) {
    ...
    private UserAccount withRoles(UserAccount user) {
        ...
        return new UserAccount(
                user.id(), user.tenantId(), user.username(), user.displayName(), user.email(), user.passwordHash(),
                user.status(), new LinkedHashSet<>(roles), user.createdAt(), user.updatedAt()
        );
    }
}
