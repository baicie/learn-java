...
public final class WorkRecordFilterValidator {
   ...
   */
  public static void validate(List<WorkRecordField> templateFields, List<DynamicFieldFilter> filters) {
  ...
  private static void validateSingle(Map<String, WorkRecordField> fieldByCode, DynamicFieldFilter filter) {
    ...
    if (!field.filterable()) {
      throw new IllegalArgumentException(
          "fieldCode '" + fieldCode + "' is not filterable");
    }
    ...
    if (!allowedOps.contains(operator)) {
      throw new IllegalArgumentException(
          "operator '" + operator + "' is not supported for fieldType '" + field.fieldType()
              + "' on field '" + fieldCode + "'");
    }
