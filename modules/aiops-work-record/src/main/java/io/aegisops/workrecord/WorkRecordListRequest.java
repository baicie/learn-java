 ...
 * 工作记录列表查询请求。
 *
 * <p>支持内置字段筛选和动态字段筛选。动态字段筛选通过 filters 参数传入，
 * 在 {@link WorkRecordFilterValidator} 中校验 fieldCode 白名单和 operator 兼容性。
 */
public record WorkRecordListRequest(
    @Min(value = 1, message = "page must be >= 1") Integer page,
    @Min(value = 1, message = "pageSize must be >= 1") @Max(value = 100, message = "pageSize must be <= 100") Integer pageSize,
    String templateId,
    List<String> status,
