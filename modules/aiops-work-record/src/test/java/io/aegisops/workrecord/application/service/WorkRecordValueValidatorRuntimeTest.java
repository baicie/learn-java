...
/**
 * 覆盖 Phase 8 设计稿“运行态”侧重点：required 缺失、未知字段、datetime 类型、dict 启用项校验。既有
...
class WorkRecordValueValidatorRuntimeTest {
  ...
  void shouldValidateRequiredField() {
    assertThatThrownBy(
            () ->
                validator.validate(
                    "t1", List.of(field("content", FieldType.TEXTAREA, true)), "{}"))
  ...
  void shouldValidateDictValueIncludingHistoricalRuntimeSubmit() {
    ...
    assertThatCode(
            () -> validator.validate("t1", List.of(selectField), "{\"priority\":\"P1\"}"))
        .doesNotThrowAnyException();
