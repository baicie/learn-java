...
class PlatformModuleServiceTest {
  @Mock
  private PlatformModuleRepository repository;
  ...
  void getOrCreatePlatform_whenNotExists_shouldCreate() {
    when(repository.findByModuleId("platform")).thenReturn(java.util.Optional.empty());
  ...
  void getOrCreatePlatform_whenExists_shouldReturnExisting() {
    PlatformModule existing = new PlatformModule("1", "platform", "平台底座", "1.0.0", true, "HEALTHY", "{}", null);
    when(repository.findByModuleId("platform")).thenReturn(java.util.Optional.of(existing));
